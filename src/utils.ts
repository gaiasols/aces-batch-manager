...
export async function getBatch(db: D1Database, batch_id: string) {
...
export function getBatchRuntimeInfo(modules: VBatchModule[], batch_id: string, batch_type = 'ASCENT', split = 1) {
...
export function getAssessorReqs(alloc?: SlotsAlloc) {
	...
	const maxdisc = !alloc
		? 0
		: [alloc.disc_slot1, alloc.disc_slot2, alloc.disc_slot3, alloc.disc_slot4].reduce((a, o) => {
				return a + o;
		  }, 0);
	const minface = !alloc
		? 0
		: Math.max(alloc?.face_slot1_size ?? 0, alloc?.face_slot2_size ?? 0, alloc?.face_slot3_size ?? 0, alloc?.face_slot4_size ?? 0);
	const maxface = !alloc
		? 0
		: [alloc?.face_slot1_size ?? 0, alloc?.face_slot2_size ?? 0, alloc?.face_slot3_size ?? 0, alloc?.face_slot4_size ?? 0].reduce(
				(a, o) => {
					return a + o;
				},
				0
...
export async function getSlotPosition(DB: D1Database, batchId: string, type: 'DISC' | 'FACE' = 'FACE') {
	...
	return (x.results as unknown as VGroup[]).reduce((acc, curr) => {
